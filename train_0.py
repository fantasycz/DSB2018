...
valid_name = 'valid_ids_fix_66'
point = None # RESULTS_DIR + '/config/checkpoint/15020_model.pth'
initial_rate = 0.01
initial_iters = 1000 * 8
...
def run_train():
    ...
    out_dir  = RESULTS_DIR + '/no_constrast'
    initial_checkpoint = point
        # None
        # RESULTS_DIR + '/test/checkpoint/8028_model.pth'
        ##
    ...
    cfg = Configuration()
    net = MaskRcnnNet(cfg).cuda() # MaskNet(cfg).cuda()
    ...
    num_iters   = initial_iters # 1000
    iter_smooth = 20
    iter_log    = 50
    iter_valid  = 100
    iter_save   = [num_iters-1]\
                   + list(range(0,num_iters,1000))#1*1000


    LR = None  #LR = StepLR([ (0, 0.01),  (200, 0.001),  (300, -1)])
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, net.parameters()),
                          lr=initial_rate/iter_accum, momentum=0.9, weight_decay=0.0001)
    ...
    '''
    weights = []
    with open(RESULTS_DIR+'/config/weights.csv', 'r') as csv_file:
        reader = csv.reader(csv_file)
        for row in reader:
            weights.append(float(row[1][1:-1]))
    weights = 1 / torch.DoubleTensor(weights)
    '''
    train_dataset = ScienceDataset(
                            train_name, mode='train',
