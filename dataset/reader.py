...
class ScienceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ['train']:
            ...
            # image = Image.open(DATA_DIR + '/color_norm/%s.png'%(id_), 'r').convert('RGB')
        ...
        if self.mode in ['test']:
            image = cv2.imread(DATA_DIR + '/zhen_fix/stage1_test/images/%s.png'%(id_), cv2.IMREAD_COLOR)
            # image = Image.open(DATA_DIR + '/zhen/stage1_test/images/%s.png'%(id_), 'r').convert('RGB')
            # image = Image.open(DATA_DIR + '/color_norm/%s.png'%(id_), 'r').convert('RGB')
            image = np.array(image)
            if self.transform is not None:
...
def run_check_dataset_reader():
    ...
    for n in iter(sampler):
        ...
        # plt.imshow(np.hstack([image,color_overlay,contour_overlay]))
        # plt.show()
        ...
        for i in range(num_masks):
            ...
            # plt.imshow(np.hstack([instance1, image1,color_overlay1,contour_overlay1]))
            # plt.show()
            # cv2.waitKey(0)
